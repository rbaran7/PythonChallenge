...
def get_neighbors(x, y, rows, columns):
    neighbor = set()
...
def find_connected_regions(x, y):
    cell_in_matrix = set([(x, y)])
    current_cell = set([(x, y)])
    while current_cell:
        next_cell = set()
        for x, y in current_cell:
            for i, j in get_neighbors(x, y, rows, columns):
...
for x in xrange(rows):
    for y in xrange(columns):
        if matrix[x][y] and (x, y) not in checked_position:
            maximum_region = max(maximum_region, find_connected_regions(x, y))
